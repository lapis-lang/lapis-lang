...
Deno.test("finiteInhabitants: a record of finites is the product of its fields", () => {
    ...
    const bool = createBoolType()
    const pair = DataType.define("Pair")
        .addVariant(
            new Variant("MkPair", [
                new Field("fst", bool),
                new Field("snd", bool),
...
Deno.test("finiteInhabitants: a function-typed field makes the type unbounded", () => {
    ...
    const fnBox = DataType.define("FnBox")
        .addVariant(new Variant("MkFnBox", [new Field("f", new FunType(nat, nat))]))
        .build()
...
Deno.test("finiteInhabitants: an 18-Bool record (2¹⁸) exceeds the ceiling; 17 Bools sit exactly at it", () => {
    ...
    const h = boolHarness()
    const wide = DataType.define("Wide")
        .addVariant(
    ...
    const narrow = DataType.define("Narrow")
        .addVariant(
            new Variant(
                "MkNarrow",
                Array.from({ length: 17 }, (_, i) => new Field(`f${i}`, h.bool)),
            ),
        )
        .build()
...
Deno.test("discharge: a failed inhabitant construction rejects the declaration — no silent holes", () => {
    ...
    const h = boolHarness()
    const unreg = DataType.define("Unreg")
        .addVariant(new Variant("U1", []), new Variant("U2", []))
        .build()
    const holder = DataType.define("Holder")
        .addVariant(new Variant("MkHolder", [new Field("u", unreg)]))
        .build()
...
Deno.test("discharge: an empty operand type discharges vacuously (0 instances)", () => {
    ...
    const h = boolHarness()
    const empty = DataType.define("Empty").build()
...
Deno.test("discharge: a variant with a zero-inhabitant field contributes nothing — without enumerating its siblings", () => {
    ...
    const big = DataType.define("Big")
        ...
        .build()
    const mixed = DataType.define("Mixed")
        .addVariant(
...
Deno.test("discharge: a residual law whose screen declines is REJECTED — zero coverage installs nothing", () => {
    ...
    const h = boolHarness()
    const streamLike = DataType.define("StreamLike")
        .addVariant(new Variant("Wrap", [new Field("inner", Family)]))
        .build()
...
Deno.test("discharge: a data type with a pattern-typed field is sampled — Empty | With(Pat) screens both variants", () => {
    ...
    const withPat = DataType.define("WithPat")
        .addVariant(
            new Variant("Empty", []),
...
Deno.test("enumerator contract: a self-typed non-recursive data field re-enters without losing the prefix", () => {
    ...
    const h = boolHarness()
    const selfBuilder = DataType.define("Self")
    const self = selfBuilder
        .addVariant(
            new Variant("Base", []),
            new Variant("Wrap", [new Field("inner", selfBuilder)]),
        )
        .build()
...
Deno.test("certified screen: a wide-flat record certifies its raised min class (7-Bool record, 128) via the RESIDUAL path", () => {
    ...
    const h = boolHarness()
    const nat = createNatType()
    const rec = DataType.define("Rec7")
        .addVariant(
...
Deno.test("certified screen: a construction yielding a foreign carrier's variant is rejected", () => {
    ...
    const victim = DataType.define("Victim")
        .addVariant(
            new Variant("Tag", [new Field("b", h.bool)]),
        )
        .build()
    const impostor = DataType.define("Impostor")
        .addVariant(new Variant("Tag", [new Field("n", DataType.define("Unused").build())]))
        .build()
