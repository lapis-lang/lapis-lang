...
The `chain` combinator (an L-attributed bind that emits the pair `[v, w]` — see lang-forma's
`chain` JSDoc; only the second value is used here) breaks this: it parses the first parser,
then — _after_ it
