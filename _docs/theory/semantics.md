...
| **derivable**     | Any type, if the fold's handler bodies fall in the primitive-law derivation fragment (characterized — `derivation.ts`)                         | Induction over the fold schema + primitive base laws (fold-fusion etc.)                       | **Proof** — implemented (`derivation.ts`) |
