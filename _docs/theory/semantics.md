...
_useful_ for optimization:

- **Identity guard:** `identity:E` on a fold means `fold_T E {Cᵢ → tᵢ} = E`. The compiler
  short-circuits: when the identity element is encountered as an argument, the fold is not entered.
  This is justified by the algebra law `f(E, x) = x`, which holds because the fold is a homomorphism
  and the law is declared and verified.

- **Horner fusion:** `distributive:sum` on a fold means `fold(⊕) ∘ fold(⊗) = fold(⊗-then-⊕)`. The
  compiler fuses two folds into one traversal. This is justified by the distributivity law, which
  holds because both folds are homomorphisms and distributivity is declared and verified.

- **Involutory cancellation:** `involutory` on an operation means `f(f(x)) = x`. The compiler
  cancels consecutive pairs in a merge pipeline. This is justified by the involutory law.
...
bialgebraic pipeline from structure to laws to performance.
...
Law checking verifies declared `properties` against generated samples. This pass runs after type
checking (it needs typed terms to generate valid samples).
...
- Recursive-field variants: one shallow recursive sample (one level deep, base case for the
  recursion).

This is _probabilistic_ checking, not complete verification — it can miss counterexamples that only
appear at greater depth. The "static where possible, dynamic when needed" philosophy accepts this:
the compiler discharges what it can, and the runtime can re-check on actual inputs.
