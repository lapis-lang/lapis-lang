...
| Production-path premise enforcement    | ✅ Complete        | All typing rules enforce premises in their production overrides (`varProd`, `variantProd`, `obsProd`, `foldProd`, `unfoldProd`, `cofoldProd` join `appProd`/`typeAppProd`/`letProd`/`opProd`); `isSubtype`/`join`/`meet` guard `undefined`; no `undefined`/sentinel-`Any` in the forest; 30 tests in `test/premises.test.ts` | #56          |
