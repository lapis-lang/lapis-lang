...
| [`theory/lapis-vs-provers.md`](./theory/lapis-vs-provers.md)         | Position statement vs. theorem provers (Agda/Coq/Lean): what the enforced fold/unfold representation gives for free (totality, canonicity, mechanical induction), what cannot be represented (partiality, dependent/HOAS propositions, coinductive proof), the provenance ladder (asserted/discharged/primitive — no external certification), and live observation of the residual |
| [`theory/lapis-vs-verse.md`](./theory/lapis-vs-verse.md)             | Point-by-point comparison with Verse (Epic Games): types-as-predicates vs types-as-shapes, ambient-unified vs stratified-minimal architectures, per-effect vs unconditional totality, the λℵ undecidability caution, what each can learn without adopting, and why the law-exploitation lane is empty in both |
